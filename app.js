...
function applyLang(id, persist = true) {
  ...
  els.walletText.textContent = t(lang, 'wallet');
  els.btnWallet.title = t(lang, 'wallet');
...
/* ---------------- wallet: balance, daily spending, reports ---------------- */
...
function loadWallet() {
  try {
    ...
    if (w && Array.isArray(w.tx)) {
      return {
        cur: typeof w.cur === 'string' && w.cur.trim() ? w.cur.trim() : '$',
...
function closeWalletPop() {
  els.walletPop.hidden = true;
  els.btnWallet.setAttribute('aria-expanded', 'false');
}
...
function initUI() {
  ...
  document.addEventListener('pointerdown', (e) => {
    ...
    if (!els.walletPop.hidden && !e.target.closest('.walletmenu')) closeWalletPop();
  });
  ...
  els.btnWallet.addEventListener('click', () => (els.walletPop.hidden ? openWalletPop() : closeWalletPop()));
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') { closePop(); closeLangPop(); els.winPop.hidden = true; els.timerPop.hidden = true; els.ipPop.hidden = true; closeWalletPop(); closeMap(); return; }
