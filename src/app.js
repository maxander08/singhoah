...
export const STRINGS = {
  ...
  'zh-Hant': {
    ...
    resync: '重新同步', search: '搜尋城市或國家…', signin: '登入', signout: '登出', account: '帳戶', settings: '設定', appearance: '外觀', homeCity: '時鐘城市', currency: '貨幣', data: '資料', resetData: '重設本地資料', resetQ: '清除此裝置上的語言、外觀、城市、貨幣與帳戶快照？錢包交易會保留。', portalNote: '登入後，您的語言、外觀、城市、貨幣與錢包將隨 Google 帳戶在任何裝置上同步。', lpSettingsSub: '語言、外觀、城市與貨幣', scribeIn: '語音語言', scribeOut: '轉寫語言', scribeStart: '開始', scribeStop: '停止', scribeListening: '聆聽中', scribeClear: '清除', scribeCopy: '複製', scribeDownload: '下載', scribeSaved: '所有變更已儲存', scribeSaving: '儲存中…', scribeWords: '字', scribeHint: '按下「開始」並說話——轉寫會出現在這裡，並可隨時編輯。', scribeNo: '此瀏覽器不支援語音辨識。', lpScribeSub: '即時課堂轉寫', lpScribe: '聽寫', language: '語言', local: '本地',
...
export class ClockCore {
  ...
  _curOff() {
    if (!this._sw) return this.offset;
    const k = (this.nowFn() - this._sw.t0) / this._sw.dur;
    if (k >= 1) { this.offset = this._sw.to; this._sw = null; return this.offset; }
...
function initUI() {
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    if (Date.now() - syncResult.at > 60 * 1000) runSync();   /* woke up: re-check drift */
  });
}
